...
/*
        ...
        else if(working_mode == CAN_SIMULATE)
        {
			...
			lcd_goto(0,0);
			lcd_puts(0, "CAN 0xFEE9", TXT, WITH_SPACE);
        	...
        	{
				CAN_ModeConfig(LPC_CAN1, CAN_RESET_MODE, DISABLE);
        		can_send_msg(LPC_CAN1, 0x00FEE900, 0x00200300, 0xE8030000);
        		CANMsgCounter++;

        		char buf[16];

				sprintf(buf, "msg : %3u", CANMsgCounter);
...
/*
				...
				if(TRUE) // time var - RIT interuptaa summeejas , +50 sekundee
				{
					...
					int AnalogValue_ADC1 = ADC_ChannelGetData(LPC_ADC,ADC_CHANNEL_1);
					//int AnalogValue_ADC2 = 2 * ADC_ChannelGetData(LPC_ADC,ADC_CHANNEL_2);

					if(abs(AnalogValue_ADC1 - previous_value) > 50 && throwed_out == FALSE) // ja izmai�a par lielu un nav jau izfiltr�ts , tad v�rt�bu atmetam
					{
						throwed_out = TRUE;
					}
					else
					{
						test++;
						UNS_8 speed = AnalogValue_ADC1 / 30;
	    				UNS_16 CANSpeed= speed;
	    				uint CANDATA = CANSpeed << 8;


	    				if(time_var1%5 == 0) // time var - RIT interuptaa summeejas , +50 sekundee
	    				{
							lcd_clear_page(1,1,0,123);
							sprintf(buf, "SPEED : %u, %08X", speed,CANDATA);
							lcd_clear_page(0,1,0,256);
							lcd_goto(1,0);
							lcd_puts(0, buf, TXT, WITH_SPACE);
		    				time_var1 = 1;
	    				}
						//lcd_clear_page(2,1,0,123);
	    				//sprintf(buf, "Speed : %4u", AnalogValue_ADC2);
	    	    		...
	    	    		can_send_msg(LPC_CAN1, 0x00FEF100, CANDATA , 0); // speed
	    	    		//can_send_msg(LPC_CAN1, 0x00F00400, AnalogValue_ADC2 & 0xFF, (AnalogValue_ADC2 >> 8) << 24);
	    		//		can_send_msg(LPC_CAN1, 0x00FEFC00, AnalogValue, 0x00000000);//Total engine hours 11=1100;
					...
					previous_value = AnalogValue_ADC1;
				}
        	}
        }
    	/*
